...
namespace Cori {
	...
	private:
		...
		void SubmitPrompt(std::string_view command);

		static int PromptCallback(ImGuiInputTextCallbackData* data);

		int OnPromptCompletion(ImGuiInputTextCallbackData* data);

		int OnPromptHistory(ImGuiInputTextCallbackData* data);

		int OnPromptAlways(ImGuiInputTextCallbackData* data);

		[[nodiscard]] int OnPromptCharFilter(ImGuiInputTextCallbackData* data) const;

		void RefreshCompletions(std::string_view line, uint64_t caret);

		[[nodiscard]] float CompletionHeight() const;

		void DrawCompletions(float height);

		[[nodiscard]] std::pair<std::string, uint64_t> BuildCompletedLine(std::string_view line, uint64_t caret, int32_t index) const;

		void MoveCompletionSelection(int32_t delta);

		void BeginCycle(std::string_view line, uint64_t caret);

		void ApplyCycleStep(ImGuiInputTextCallbackData* data);

		[[nodiscard]] std::string RenderCandidate(int32_t index) const;
		...
		static constexpr uint64_t s_MaxHistory{ 128 };
		static constexpr int32_t s_MaxVisibleCompletions{ 8 };
		...
		std::vector<Core::ConsoleCompletion> m_Completions;
		std::string m_CompletionLine;
		std::string m_CompletionScratch;
		uint64_t m_CompletionCaret{ 0 };
		uint64_t m_CycleBegin{ 0 };
		uint64_t m_CycleEnd{ 0 };
		int32_t m_CompletionIndex{ -1 };
		int32_t m_AcceptIndex{ -1 };
