	...
	namespace Graphics {
		...
		void VulkanTextureManager::RegisterAtSlot(const Core::Handle<Texture2> handle) {
			...
			RenderThreadCommandQueue::Push([handle]() mutable {
				...
				CORI_PROFILER_ZONE_TEXT_P(Cori::ProfileParts::RenderingAssets, "Outcome: Emplaced and placeholder assigned.");
			});
		}
		...
		void VulkanTextureManager::Load(const Core::Handle<Texture2> handle, const Core::AssetID id, const uint32_t gen, const uint32_t vectorKey, std::filesystem::path path, std::string name) {
			...
			CORI_PROFILER_ZONE_TEXT_FP(Cori::ProfileParts::RenderingAssets, "Handle=[%u, %u] gen=%u id=%llu", handle.GetIndex(), handle.GetVersion(), gen, id);
			...
			Core::Application::SubmitWorkerTask([path = std::move(path), name = std::move(name), handle, id, gen, vectorKey]() mutable {
				CORI_PROFILE_SCOPE_CP(Cori::ProfileParts::RenderingAssets, "Worker Task: Texture parse/decode/creation", Cori::ProfileColors::Decode);
				...
				CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Debug, Cori::ProfileColors::Decode, "%s Handle=[%u, %u] worker begin (parse/decode/create), (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle.GetIndex(), handle.GetVersion(), gen, static_cast<unsigned long long>(id));
				...
				auto FinalizeLoad = [](const Core::Handle<Texture2> handle_, const Core::AssetID id_, const uint32_t gen_, const uint32_t vectorKey_, std::expected<WorkerPayload, ErrorCode>&& payload) {
					if (payload) {
						RenderThreadCommandQueue::Push([id_, handle_, gen_, payload = std::move(payload.value())]() mutable {
							...
							CORI_PROFILER_ZONE_TEXT_FP(Cori::ProfileParts::RenderingAssets, "Handle=[%u, %u] gen=%u id=%llu", handle_.GetIndex(), handle_.GetVersion(), gen_, id_);
							...
							if (!IsHandleValid(handle_)) {
								...
								CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Warning, Cori::ProfileColors::Stale, "%s Handle=[%u, %u] finalize dropped: handle invalid (decoded image discarded), (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle_.GetIndex(), handle_.GetVersion(), gen_, static_cast<unsigned long long>(id_));
								return;
							}

							const uint32_t currentGen = Get().m_HandleAllocator.GetGeneration(handle_);
							if (currentGen != gen_) {
								CORI_PROFILER_ZONE_TEXT_FP(Cori::ProfileParts::RenderingAssets, "Outcome: STALE (expected gen=%u, current gen=%u), dropped decoded image", gen_, currentGen, id_);
								CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Warning, Cori::ProfileColors::Stale, "%s Handle=[%u, %u] finalize dropped: STALE (expected gen=%u, current gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle_.GetIndex(), handle_.GetVersion(), gen_, currentGen, static_cast<unsigned long long>(id_));
								return;
							}
							...
							if (success) {
								...
								CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Error, Cori::ProfileColors::Destroy, "%s Handle=[%u, %u] finalize: UpdateTexture FAILED, reverted to MISSING placeholder, (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle_.GetIndex(), handle_.GetVersion(), gen_, static_cast<unsigned long long>(id_));
							}
						...
						RenderThreadCommandQueue::Push([id_, handle_, gen_, vectorKey_]() {
							...
							if (!IsHandleValid(handle_)) {
								...
								CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Warning, Cori::ProfileColors::Stale, "%s Handle=[%u, %u] finalize(fail) skipped: handle invalid, (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle_.GetIndex(), handle_.GetVersion(), gen_, static_cast<unsigned long long>(id_));
								return;
							}
							...
							if (currentGen != gen_) {
								...
								CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Warning, Cori::ProfileColors::Stale, "%s Handle=[%u, %u] finalize(fail) skipped: STALE (expected gen=%u, current gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle_.GetIndex(), handle_.GetVersion(), gen_, currentGen, static_cast<unsigned long long>(id_));
								return;
							}
							...
							CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Error, Cori::ProfileColors::Destroy, "%s Handle=[%u, %u] LOAD FAILED, status=LoadFailed, MISSING placeholder shown, (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle_.GetIndex(), handle_.GetVersion(), gen_, static_cast<unsigned long long>(id_));
						});
					}
				};
				...
				JsonAssetDataCombined data;
				{
					...
					if (readError != glz::error_code::none) {
						...
						CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Error, Cori::ProfileColors::Destroy, "%s Handle=[%u, %u] LOAD FAILED: cannot open asset file '%s', (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle.GetIndex(), handle.GetVersion(), path.c_str(), gen, static_cast<unsigned long long>(id));
					...
					if (parseError) {
						...
						CORI_PROFILER_MSG_SCFP(Cori::ProfileParts::RenderingAssets, Cori::Error, Cori::ProfileColors::Destroy, "%s Handle=[%u, %u] LOAD FAILED: asset json '%s' parse error, (gen=%u) (id=%llu)", CORI_CLEAN_TYPE_NAME(Texture2), handle.GetIndex(), handle.GetVersion(), path.c_str(), gen, static_cast<unsigned long long>(id));
