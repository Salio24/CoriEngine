...
namespace Cori {
	namespace Graphics {
		...
		std::size_t VulkanTextureManager::SCIHasher::operator()(const vk::SamplerCreateInfo& info) const noexcept {
			uint64_t hash;
		...
		VulkanTextureManager::WorkerPayload::~WorkerPayload() {
			m_Image.Destroy();
		}

		VulkanTextureManager::WorkerPayload::WorkerPayload(WorkerPayload&& other) noexcept {
			m_Image = other.m_Image;
			m_PixelData = std::move(other.m_PixelData);
			other.Release();
		}

		VulkanTextureManager::WorkerPayload& VulkanTextureManager::WorkerPayload::operator=(WorkerPayload&& other) noexcept {
			if (m_Image.m_Image) {
				m_Image.Destroy();
			}
		...
		void VulkanTextureManager::WorkerPayload::Release() {
