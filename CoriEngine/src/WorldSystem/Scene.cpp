...
namespace Cori {
	...
	std::expected<void, CoriError<>> Scene::AddEntityToCache(const Entity entity, const Utility::StringHash32 key) {
		if (m_EntityCache.contains(key)) {
			return std::unexpected(CoriError("Entry with the given key already exists, make sure you're not reusing the key. It's also possible (but very unlikely), that you got a hash collision, try to change the key a bit."));
		}
	...
	std::expected<Entity, CoriError<>> Scene::GetEntityFromCache(const Utility::StringHash32 key) {
		if (!m_EntityCache.contains(key)) {
			return std::unexpected(CoriError("No entity with the specified key found in scene local cache."));
		}
		if (!m_Registry.valid(m_EntityCache.at(key))) {
			m_EntityCache.erase(key);
			return std::unexpected(CoriError("Entity at the specified key is invalid, removing this entry."));
		}
	...
	std::expected<Entity, CoriError<>> Scene::FindEntity(const std::string& name) {
		...
		return std::unexpected(CoriError("No entity found with the specified name."));
	}
	std::expected<Entity, CoriError<>> Scene::FindEntity(const std::string& name, const Utility::HashedTag64& tag) {
		...
		return std::unexpected(CoriError("No entity found with the specified name and tag."));
	}
